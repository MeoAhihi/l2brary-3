...
export function EventCertificateForm({ user }: { user: GetOneUserResponse }) {
  const userId = user.id;
  const items = user.eventCertificates;
  ...
  return (
    <>
      <h2 className="my-3 font-bold">Sự kiện đã tham gia</h2>
      <div onSubmit={handleSubmit} className="flex items-center gap-2">
        <Input
          value={value}
          onChange={(e) => setValue(e.target.value)}
          placeholder="Thêm huy hiệu sự kiện mới"
          className="max-w-xs"
        />
        <Button
          onClick={() =>
            updateUser.mutate({
              id: userId,
              data: { eventCertificates: [...(items ?? []), value] },
            })
          }
        >
          Lưu
