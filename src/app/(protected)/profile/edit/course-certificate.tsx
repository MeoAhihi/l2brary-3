...
export function CourseCertificateForm({ user }: { user: GetOneUserResponse }) {
  ...
  const items = user.courseCertificates;
  ...
  return (
    <>
      <h2 className="my-3 font-bold">Chứng nhận Lớp học</h2>
      <div onSubmit={handleSubmit} className="flex items-center gap-2">
        <Input
          value={value}
          onChange={(e) => setValue(e.target.value)}
          placeholder="Thêm huy hiệu Lớp học mới"
          className="max-w-xs"
        />
        <Button
          onClick={() =>
            updateUser.mutate({
              id: userId,
              data: { courseCertificates: [...(items ?? []), value] },
            })
          }
        >
          Lưu
