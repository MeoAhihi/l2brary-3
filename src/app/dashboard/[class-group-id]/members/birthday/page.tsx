...
].map(member => ({ avatarUrl: "lorem.photos/500", name: member.fullName, birthday: member.birthday }))
    ...
    <>
      ...
      {["Tháng 1",
        ...
        <div key={index} className="max-w-fit">
          ...
          <MemberList
            className="my-4"
            members={data.filter(member => member.birthday.getMonth() === index) || []}
            optionalFields={[{ key: "birthday", label: "Ngày sinh", render: birthday => <p>{birthday.toLocaleDateString("en-GB")}</p> }]}
            showHeader={false}
          />
        </div>
      ))}
